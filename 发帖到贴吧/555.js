...
function dkkwxz() {
    ...
    if (排行按钮) {
        ...
        log("点击了")
        //press(排行按钮.bounds().centerX(), 排行按钮.bounds().centerY(), 200)
    }
    sleep(2000);
...
function 上滑动作() {
    ...
    var y0 = device.height / 4 * 3 x0 = x0 + rndNum(-30, 100) y0 = y0 + rndNum(-30, 100) // log('x0,y0',x0,y0)

    var angle = 0
    var x = 0
    var y = 0
    for (let i = 3; i < 40; i++) {
        y = x * tan(angle) y = Math.floor(y) // log(y)

        if ((y0 - y) < 0) {
            break
        }
        var xy = [x0 + x, y0 - y] xyArr.push(xy) x += 5;
        angle += 3
    }
    log(xyArr) gesture.apply(null, xyArr)
