...
var accountList = ["软剑", "ko😁boy", "仲夏生活日常", "《结贝号》", "发烧哥💯", "省事业", "听歌发烧哥😁😁", "sxp198812", "瞰主业联细", "逛逛有收获☜"];
...
function fatie() {
    ...
    var obfuscatedImageView1 = id("obfuscated").className("android.widget.ImageView").clickable(true).depth(2).findOne(1000);
    if (obfuscatedImageView1) {
        obfuscatedImageView1.click();
    }

    var obfuscatedImageView2 = id("obfuscated").className("android.widget.ImageView").clickable(true).depth(4).findOne(1000);
    if (obfuscatedImageView2) {
        obfuscatedImageView2.click();
    }

    // id("obfuscated").className("android.widget.ImageView").clickable(true).depth(2).findOne().click();
...
function 读取内容输入函数() {
    var filePath = "/sdcard/zjl.txt"; // 文件路径
    //var filePath = "/sdcard/cenxiang.txt"; // 文件路径

    var fileContent = files.read(filePath); // 读取文件内容

    if (fileContent) {
        var lines = fileContent.split("**"); // 按换行符分割每一行
        var randomIndex = random(1, lines.length - 1); // 随机选择一行

        var selectedLine = lines[randomIndex];
        var parts = selectedLine.split("|"); // 再次以"|"分隔符分割标题和内容

        if (parts.length >= 2) {
            var title = parts[0].substring(0, 30);
            var content = parts[1];
            ...
            // 输入标题和内容到两个输入框
            // var titleInput = id("your_first_input_id").findOne(); // 用你的第一个输入框的ID替换"your_first_input_id"

            var tleInput = className("android.widget.EditText").find();
            if (tleInput.length > 0) {
                tleInput[0].click();
                console.log("输入标题:" + title);
                sleep(1000);
                tleInput[0].paste();
                // tleInput.setText(title);
            };
            sleep(1000);
            //设置内容格式
            ...
            var contentInput = className("android.widget.EditText").find();
